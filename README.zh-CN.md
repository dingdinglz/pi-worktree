...
`/wt → new`（或不带任务描述的 `/wt new`）中的任务输入支持 `/` 补全已加载的扩展命令、技能和 prompt 模板，以及相对当前目录的 `@` 文件引用。用 ↑/↓ 选择，Tab 或 Enter 接受候选，再按 Enter 提交。Esc 先关闭补全列表，再按一次取消。补全只插入任务文本，不会在当前会话执行命令。任务描述仍须为单行，最多 500 字符。
